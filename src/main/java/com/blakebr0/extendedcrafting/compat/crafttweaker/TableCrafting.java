...
public final class TableCrafting implements IRecipeManager<ITableRecipe> {
	...
	public static void addShaped(String name, int tier, IItemStack output, IIngredient[][] inputs) {
		...
		if (tier > 4 || tier < 0) {
			...
			CraftTweakerAPI.getLogger(ExtendedCrafting.MODID).error("Unable to assign a tier to the Table Recipe for stack " + output.getCommandString() + ". Tier cannot be greater than 4 or less than 0.");
		}
